...
sprout = 1
for i in range(1):
    path = r"temp/temp query/3DAG/"
    SRC_vs_BRC(tree=temp,path=path,num=num,sprout=sprout+i,show=False,one_file=False,duplicates=False)
    try:
        statistics(path+"Without Duplicates/{} - {}/".format((i+sprout), f"{num:,}"),graph=True)
    except Exception:
        print("Error with statistics!!!")
        continue
    L2norm(path+"Without Duplicates/{} - {}/".format((i+sprout), f"{num:,}"))
...
for i in range(1):
    DAGpath = r'temp/temp query/3DAG/Without Duplicates/{} - {}/'.format((i+sprout),f"{num:,}")
    KDpath = r'temp/temp query/KD/Without Duplicates/{} - {}/'.format((i+sprout),f"{num:,}")

    lvl_diff(DAGpath=DAGpath,KDpath=KDpath,title=f"Spatial {(i+sprout)} - {num:,}",show=False)
