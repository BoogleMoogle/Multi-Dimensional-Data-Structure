...
class DAGTree:
    def __init__(self, points=[], depth=0, axis=0, split_value=None, bbox=[], left=None, right=None, middle=None, middle_child=False, parent=[], cuttoff=1):
        ...
        # if self != None and self.data_size > self.cuttoff:
        #     self.split()
        ...
        if self.parent != [] and self.parent[0].data_size == self.data_size:
            ...
            if self.axis == 0:  #this is on the x axis
                ...
                if middle != [] and left != [] and right != []:
                    ...
                    self.middle = DAGTree(middle, depth=self.depth+1, axis=1, bbox=[middle[0][0],self.bbox[1],middle[len(middle)-1][0],self.bbox[3]], parent=[self], middle_child=True, cuttoff=self.cuttoff)
                    # self.middle = self.middle.connect()
    ...
    def looking(self,pTmp,alist=[]):  #pTmp is the original node we are comparing, self is started as the root node (most top). It is looking through every single node, until one is found.
        ...
        # if self.left != None:
        #     # print("Left")
        #     return self.left.looking(pTmp)
        
        # if self.right != None:
        #     # print("Right")
        #     return self.right.looking(pTmp)
        
        # if self.middle != None:
        #     # print("Middle")
        #     return self.middle.looking(pTmp)
    ...
    def connect(self, temp=None):
        ...
        if self.parent != []:
            ...
            temp = root.looking(pTmp,alist=[])  #gets a node with matching bbox that isn't the same node
            ...
            if temp != None and temp != []:     #if we found a node
                ...
                #now need to get rid of this self
                if self.left != None:
                    self.left = None
                if self.right != None:
                    self.right = None
...
def SRC_vs_BRC(tree=None,num=1,sprout=None,path=None,show=False,duplicates=False,starting_per=.30,interval=None):
    ...
    coeffs_list = []
    #Setting Coeffs
    for j in range(int(round((starting_per*100) / interval,6))+1):
    ...
    for i in range(len(coeffs_list)):
        ...
        while j != num:
            ...
            r = 1
            xmin = round(random.uniform(tree.bbox[0],tree.bbox[2]-(coeff_numx*r)),6)
            xmax = round(xmin + coeff_numx,6)
            ymin = round(random.uniform(tree.bbox[1],tree.bbox[3]-(coeff_numy*1/r)),6)
            ymax = round(ymin + coeff_numy,6)
...
tree = DAGTree(points, cuttoff=1, axis=0)
...
#     # save_query(tree=tree,num=1,path=SRC_path,SRC=True,BRC=False,save=True,query_list=pre_list)
    
#     # BRC_path = f"{path}BRC_3x3.csv"
#     # save_query(tree=tree,num=1,path=BRC_path,SRC=False,BRC=True,save=True,query_list=pre_list)

#     SRC_vs_BRC(tree=tree,num=num,sprout=sprout+i,path=path,show=False,duplicates=False,starting_per=starting_per,interval=interval)
