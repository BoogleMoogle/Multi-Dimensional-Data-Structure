...
class KDTree:
    ...
    def split(self):
        ...
        if self.data_size <= 4:
            ...
            return self.left.split() and self.right.split()
...
# csv_data = points_from_file(path)
...
# print(f"Minimum of Points (X,Y): ({min(points[0])}, {min(points[1])})")
# print(f"Maximum of Points (X,Y): ({max(points[0])}, {max(points[1])})")
# print(f"Mean of Points (X,Y): ({sum(points[0])/len(points)}, {sum(points[1])/len(points)})")

# points = make_points(num=2031,sprout=2,aRang=0,bRang=30)
points = make_points(num=500,sprout=2,aRang=0,bRang=30)
temp = KDTree(points)
# temp.print_tree()
